...
@Component(
	property = {
		"class.name=com.liferay.learning.model.impl.BookImpl", "table.name=Book"
	},
...
public class BookModelArgumentsResolver implements ArgumentsResolver {
	...
	public Object[] getArguments(
		...
		if (!checkColumn || (columnBitmask == 0)) {
			return _getValue(bookModelImpl, finderPath, original);
		}
	...
	private static Object[] _getValue(
		BookModelImpl bookModelImpl, FinderPath finderPath, boolean original) {
		...
		for (int i = 0; i < arguments.length; i++) {
			...
			if (original) {
				value = bookModelImpl.getColumnOriginalValue(columnName);
			}
			else {
				value = bookModelImpl.getColumnValue(columnName);
			}
...
// LIFERAY-SERVICE-BUILDER-HASH:-1554042614